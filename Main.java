...
class Main {
  ...
  /*
   * iterativeInterestCalc method uses a for loop to calulate the new balance in
   * your investment account.
   */
  public static double iterativeInterestCalc(double amount, double rate, int years) {
    /*
     * Take a percentage of the amount to get the compound interest by multiplying
     * the amount by the rate. Then add that compound interest to the amount to get
     * the updated amount. Do this calculation for each year the investment stays in
     * the account. For an investment in the account for 3 years, you would perform
     * this calulation 3 times.
     */
  ...
  // Start coding here!!
  /*
   * recursiveInterestCalc method uses recursion to calulate the new balance in
   * your investment account
   */
  ...
  public static double recursiveInterestCalc(double amount, double rate, int years) {
    if (years == 0)
      return amount;
    return recursiveInterestCalc(amount += amount * rate, rate, years - 1);
  }
}